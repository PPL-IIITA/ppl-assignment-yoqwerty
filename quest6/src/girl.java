...
public class girl{
  ...
  */
     public void ghapp() throws IOException, BiffException{
        hapig=new Vector();
         bu=new boy();
        ...
        for( i=0;i<couple.getRows()-1;i++,z++){
           ...
           for(j=1;j<=girll.getRows();j++){//finding that name in girl data
            ...
            if(natureg==1){
                hap=(int) (Math.log10((int) bu.costrec.elementAt(i)-m_cost)+2*((int) bu.luxrec.elementAt(i)));
               // System.out.println(hap  + "n=1");
            }
            else if(natureg==2){
                hap=Math.abs((int)bu.costrec.elementAt(i)- (int)bu.valrec.elementAt(i)- m_cost);
              ...
              //  System.out.println("bu_costrec= " + (int)bu.costrec.elementAt(i));
               // System.out.println("m_cost= " + m_cost);
                int now;
                if(Math.exp( (int)bu.costrec.elementAt(i) - m_cost ) > 10000)
                    now = 10000;
                else 
                    now = (int) Math.exp( (int)bu.costrec.elementAt(i) - m_cost );
                hap=now;
     ...
     /*  int f=0;
         for(j=1;j<=girll.getRows();j++){//finding that name in girl data
                ...
                if(hp.ngirl.elementAt(f++).equals(n)){
                    break;
                }
            }
            g=girll.getCell(5,j);//getting nature of girl
